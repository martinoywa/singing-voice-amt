...
Download from Zenodo (see links already listed in the minimal README on GitHub).
...
1.	Transcribe baseline vocal audio
   -  For each dataset/wav/<name>.wav: • Basic Pitch → amt_dataset/baselines/basic_pitch/<name>.mid
      • Run MT3 → amt_dataset/baselines/MT3/<name>.mid
2. Transcribe timbre-transferred audio
- For each instrument and each file in tt_dataset/<instrument>/<name>.wav:
	•	Run Basic Pitch → amt_dataset/converted/basic_pitch/<instrument>/<name>.mid
	•	Run MT3 → amt_dataset/converted/MT3/<instrument>/<name>.mid
